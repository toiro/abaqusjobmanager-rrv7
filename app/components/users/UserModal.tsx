...
export function UserModal({ 
  ...
  const config: BaseDialogConfig = {
    title: isCreateMode ? "Add New User" : `Edit User: ${user?.display_name || ''}`,
  ...
  const isFormValid = displayName.trim().length >= 2 && 
                     parseInt(maxConcurrentJobs) >= 1 && parseInt(maxConcurrentJobs) <= 50;

  // Hidden fields for edit mode
  const hiddenFields = isEditMode && user?.id ? { user_id: user.id } : undefined;
