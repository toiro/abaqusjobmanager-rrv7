...
import { TABLE_HEADERS, INFO_MESSAGES, BUTTONS } from "~/lib/messages";
...
interface JobTableProps {
	...
	onJobAction?: (
		jobId: number,
		action: "view" | "edit" | "delete" | "cancel",
	) => void;
	loading?: boolean;
			...
			{isMounted && enableRealTimeUpdates && (
				<div className="px-4 py-2 bg-muted/30 border-b text-xs flex items-center justify-between">
					...
					<div className="flex items-center space-x-2">
						<div
							className={`h-2 w-2 rounded-full ${getConnectionDotColor(connectionState)}`}
						></div>
				...
				<TableBody>
					{currentJobs.map((job) => (
						<TableRow key={job.id}>
							...
							<TableCell className="text-sm">{job.user_id || "-"}</TableCell>
