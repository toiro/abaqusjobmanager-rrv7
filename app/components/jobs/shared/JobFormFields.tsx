...
import { FORM_LABELS, PLACEHOLDERS, ERROR_MESSAGES } from "~/lib/messages";
import { calculateLicenseTokens } from "~/lib/services/license";
import { formatCpuCoresError } from "~/utils/formatting";
import type { User, Node } from "~/lib/core/types/database";
      ...
      >
        ...
        {nodes.map((node) => (
          <option key={node.id} value={node.id || ''}>
            {node.name} ({node.cpu_cores_limit} cores max) - {node.status === 'available' ? '✅ Available' : '⚠️ Unavailable'}
          </option>
        ))}
      </Select>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <p className="text-sm text-yellow-800">
                <strong>Warning:</strong> The selected node "{selectedNode?.name}" is currently unavailable. 
                The job will be queued until the node becomes available.
              </p>
            </div>
          </div>
        </div>
...
export function CpuCoresField({ 
  ...
  const isValidCores = selectedNode && coresNumber >= 1 && coresNumber <= maxCores;
