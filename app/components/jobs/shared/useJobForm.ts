...
export function useJobForm({
  ...
  }, [isModalOpen, initialJobName, initialUserId, initialNodeId, initialCores, initialPriority]);
  ...
  useEffect(() => {
    if (actionData?.error || (actionData && !actionData.success)) {
      ...
      if (actionData?.intent === 'create-job' || actionData?.intent === 'edit-job') {
        onClose();
      }
    }
  ...
  const validateForm = (
    ...
    const isValidCores = selectedNodeId && selectedNode && coresNumber >= 1 && coresNumber <= selectedNode.cpu_cores_limit;

    let isFormValid = jobName.trim().length >= 3 && 
                     selectedUserId && 
                     selectedNodeId && 
                     isValidCores &&
                     canEdit;
