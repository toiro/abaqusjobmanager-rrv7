...
import { Button, Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter, Alert, AlertDescription } from "~/components/ui";
import { BUTTONS, INFO_MESSAGES } from "~/lib/messages";
...
function JobInfoSection({ job, actionType }: { job: Job; actionType: ActionType }) {
  return (
    <Alert>
      <AlertDescription>
...
function StatusWarning({ job, actionType }: { job: Job; actionType: ActionType }) {
...
function ValidationWarning({ canPerformAction, actionType }: { canPerformAction: boolean; actionType: ActionType }) {
  ...
  return (
    <Alert variant="destructive">
      <AlertDescription>
        {config.cannotPerformMessage}
      </AlertDescription>
    </Alert>
  );
}

function ActionDataDisplay({ actionData }: { actionData?: JobActionDialogProps['actionData'] }) {
  ...
  return (
    <>
      ...
      {actionData.success && (
        <Alert>
          <AlertDescription>
            {typeof actionData.success === 'string' ? actionData.success : 'Action completed successfully'}
          </AlertDescription>
        </Alert>
      )}
    </>
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          <ValidationWarning canPerformAction={canPerformAction} actionType={actionType} />
