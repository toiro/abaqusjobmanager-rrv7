      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          <Alert>
            <AlertDescription>
              <div className="space-y-1">
                <p><strong>Node Name:</strong> {node.name}</p>
                <p><strong>Hostname:</strong> {node.hostname}</p>
                <p><strong>CPU Cores:</strong> {node.cpu_cores_limit}</p>
                <p><strong>Status:</strong> {node.is_active ? 'Active' : 'Inactive'}</p>
                <p><strong>Created:</strong> {node.created_at ? new Date(node.created_at).toLocaleString() : '-'}</p>
              </div>
            </AlertDescription>
          </Alert>
          ...
          {hasActiveJobs && (
            <Alert variant="destructive">
              <AlertDescription>
                This node has active jobs running. Please wait for jobs to complete or cancel them before deletion.
              </AlertDescription>
            </Alert>
          )}
          ...
          {actionData?.success && (
            <Alert>
              <AlertDescription>
                {typeof actionData.success === 'string' ? actionData.success : 'Node deleted successfully'}
              </AlertDescription>
            </Alert>
          )}
        </div>
