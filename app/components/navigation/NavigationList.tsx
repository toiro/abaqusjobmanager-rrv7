...
import { isActiveRoute, addTokenToHref, NavigationStyles } from "./NavigationUtils";
...
function NavigationLink({ item, href, isActive, variant }: NavigationLinkProps) {
  if (variant === 'vertical') {
    return (
      <Link
        to={href}
        className={cn(
          ...
          isActive ? NavigationStyles.verticalActive : NavigationStyles.verticalInactive
        )}
...
export function NavigationList({ 
  ...
  return (
    <nav className={cn(containerClass, className)}>
      {items.map((item) => {
        const isActive = isActiveRoute(location.pathname, item.href, item.exactMatch);
