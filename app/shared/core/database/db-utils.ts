...
export function getDb(): Database {
	try {
		...
		getLogger().error("Database: Failed to get database connection", { error });
		throw error;
	}
}
...
export function executeQuery(
	...
	try {
		...
		getLogger().debug("Database: SQL query executed", {
		...
		getLogger().error(`Database: SQL query failed`, {
			sql,
			params,
			error: errorMessage,
		});
	...
	try {
		...
		if (single) {
			...
			if (!validated.success) {
				getLogger().warn("Database: Single query result validation failed", {
					sql,
		...
		getLogger().error("Database: SELECT query failed", { sql, params, error });
		return single ? null : [];
	}
}
...
export function buildUpdateSQL(
	...
	const fields = Object.keys(data).filter(
		(key) => data[key] !== undefined && key !== idField,
	);
...
export function handleDbError(
	...
	getLogger().error(`Database: Database operation failed: ${operation}`, {
		...context,
		error: errorMessage,
	});
...
export function safeDbOperation<T>(
	...
	try {
		...
		return handleDbErrorWithFallback(
			error,
			operationName,
			fallbackValue,
			context,
...
export function logDbSuccess(
	...
	getLogger().info(
		`Database: Database operation successful: ${operation}`,
		context,
	);
}
