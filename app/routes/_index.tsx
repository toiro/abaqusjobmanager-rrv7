...
export async function loader() {
	...
	getLogger().info("Routes: Jobs page data loaded", {
...
function validateJobStatus(
	...
	if (!allowedStatuses.includes(job.status)) {
		throw new ValidationError(
			`Cannot ${action} job with status: ${job.status}`,
		);
	}
}
...
async function handleCreateJob(formData: FormData): Promise<Response> {
	...
	getLogger().info("Routes: Job created successfully", {
		jobId,
...
async function handleEditJob(formData: FormData): Promise<Response> {
	...
	getLogger().info("Routes: Job updated successfully", {
		jobId: job_id,
		jobName: jobData.name,
	});
	return success(
		{ message: "Job updated successfully", intent: "edit-job" },
		SUCCESS_MESSAGES.JOB_UPDATED,
	);
}
...
async function handleDeleteJob(formData: FormData): Promise<Response> {
	...
	getLogger().info("Routes: Job deleted successfully", {
		jobId: job_id,
		jobName: existingJob.name,
	});
	return success(
		{ message: "Job deleted successfully", intent: "delete-job" },
		SUCCESS_MESSAGES.JOB_DELETED,
	);
}
...
async function handleCancelJob(formData: FormData): Promise<Response> {
	...
	const cancelResult = jobRepository.updateJobStatus(
		job_id,
		"failed",
		"Cancelled by user",
	);
	...
	getLogger().info("Routes: Job cancelled successfully", {
		jobId: job_id,
		jobName: existingJob.name,
	});
	return success(
		{ message: "Job cancelled successfully", intent: "cancel-job" },
		SUCCESS_MESSAGES.JOB_CANCELLED,
	);
}
