import { emitSystemEvent } from "~/server/services/sse/sse.server";
import { getSSEEventEmitter } from "~/server/services/sse/sse-event-emitter.server";
import {
	isValidChannel,
	type SSEChannel,
	type AnyTypedSSEEvent,
	createTypedSSEEvent,
...
export async function loader({ request }: Route.LoaderArgs) {
	...
	if (!isValidChannel(channelParam)) {
		getLogger().warn("Invalid SSE channel requested", {
			context: "api.events",
	...
	const stream = new ReadableStream({
		start(controller) {
			...
			const connectionEvent: AnyTypedSSEEvent = {
				type: "connected",
				channel,
			...
			getLogger().info("SSE connection established", {
				context: "api.events",
				channel,
			});
			...
			const listener = (eventData: unknown) => {
				...
				if (!isConnectionActive) {
					getLogger().debug("SSE data ignored - connection closed", {
						channel,
					});
					return;
				}
				...
				try {
					...
					getLogger().debug("SSE data sent to client", {
						context: "api.events",
						channel,
					...
					if (
						...
						getLogger().debug("SSE connection closed during data send", {
							channel,
						});
						...
						getLogger().error("Failed to send SSE data", {
							context: "api.events",
							channel,
			...
			getLogger().debug("SSE listener registered", {
				context: "api.events",
				channel,
			...
			const keepAlive = setInterval(() => {
				...
				try {
					const pingEvent: AnyTypedSSEEvent = {
						type: "ping",
						channel,
						timestamp: new Date().toISOString(),
					};
					...
					getLogger().debug("SSE ping sent", {
						context: "api.events",
						channel,
					});
				} catch (error) {
					getLogger().debug("SSE ping failed - connection closed", {
						channel,
						error: error instanceof Error ? error.message : "Unknown error",
					});
					isConnectionActive = false;
			...
			request.signal?.addEventListener("abort", () => {
				...
				getLogger().info("SSE connection closed", {
					context: "api.events",
					channel,
				...
				try {
					...
					getLogger().debug("Could not emit disconnection event", {
						channel,
						error: error instanceof Error ? error.message : "Unknown error",
					});
				}
