...
> {
  ...
  loader?(args: { request: Request; params: TParams }): Promise<TLoaderData> | TLoaderData;
  action?(args: { request: Request; params: TParams }): Promise<TActionData> | TActionData;
...
export class TypedApiResponse {
  ...
  static serverError(
    ...
    getLogger().error('Server error in API response', 'Routes', { message, details });
