 ...
 */
export class JobRepository extends BaseRepository<PersistedJob, CreateJob, UpdateJob> {
  protected readonly tableName = 'jobs';
  protected readonly entitySchema = PersistedJobSchema;
  protected readonly createSchema = JobSchema.omit({ id: true, created_at: true, updated_at: true });
  ...
  findActiveJobs(): PersistedJob[] {
    const sql = "SELECT * FROM jobs WHERE status IN ('waiting', 'starting', 'running') ORDER BY created_at ASC";
    return this.findByCondition(sql, []);
  }

  findCompletedJobs(): PersistedJob[] {
    const sql = "SELECT * FROM jobs WHERE status IN ('completed', 'failed') ORDER BY created_at DESC";
    return this.findByCondition(sql, []);
  }

  updateJobStatus(id: number, status: Job['status'], errorMessage?: string): boolean {
    try {
  ...
  private findByCondition(sql: string, params: any[]): PersistedJob[] {
    ...
    return safeDbOperation(
      () => selectQuery(sql, params, this.entitySchema, false, 'Database') as PersistedJob[],
  ...
  private async triggerLicenseUpdate(jobId: number): Promise<void> {
    try {
      const { onJobCreated } = await import("../../services/license/license-usage-service.server");
      onJobCreated(jobId);
    } catch (error) {
      // License update is not critical, just log
      const { getLogger } = await import("../logger/logger.server");
      getLogger().warn('Failed to trigger license update', 'JobRepository', { jobId });
    }
  }
}
