 ...
 */
export class NodeRepository extends BaseRepository<PersistedNode, CreateNode, UpdateNode> {
  ...
  findAvailableNodes(): PersistedNode[] {
    const sql = "SELECT * FROM nodes WHERE is_active = 1 AND status = 'available' ORDER BY name";
    return this.findByCondition(sql, []);
  }
  ...
  updateNodeStatus(id: number, status: Node['status']): boolean {
    try {
      ...
      if (result.success && result.result.changes > 0) {
        this.logStatusChange(id, currentNode, previousStatus || 'unknown', status);
  ...
  private findByCondition(sql: string, params: any[]): PersistedNode[] {
    ...
    return safeDbOperation(
      () => selectQuery(sql, params, this.entitySchema, false, 'Database') as PersistedNode[],
  ...
  private logStatusChange(
    ...
    if (previousStatus !== newStatus) {
      ...
      if (newStatus === 'unavailable') {
        logger.warn('Node status changed to unavailable', 'NodeStatus', {
          nodeId: id,
      ...
      } else if (newStatus === 'available' && previousStatus === 'unavailable') {
        logger.info('Node recovered from unavailable status', 'NodeStatus', {
          nodeId: id,
          nodeName: currentNode?.name,
          hostname: currentNode?.hostname,
          previousStatus,
