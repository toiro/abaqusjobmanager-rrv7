...
export const httpLogger = (): MiddlewareHandler => {
	return async (c, next) => {
		...
		// Log request start
		getLogger(LOG_CATEGORY).info("HTTP Request started", {
			method,
			url,
			userAgent,
			ip,
		...
		if (logLevel === "error") {
			getLogger(LOG_CATEGORY).error("HTTP Request completed", {
				type: "HTTP_RESPONSE",
				...logData,
			});
		} else if (logLevel === "warn") {
			getLogger(LOG_CATEGORY).warn("HTTP Request completed", {
				type: "HTTP_RESPONSE",
				...logData,
			});
		} else {
			getLogger(LOG_CATEGORY).info("HTTP Request completed", {
				type: "HTTP_RESPONSE",
				...logData,
			});
		}
		...
		if (duration > 1000) {
			getLogger(LOG_CATEGORY).info("Slow HTTP request detected", {
				method,
				url,
				status,
				requestId,
...
export const simpleHttpLogger = (): MiddlewareHandler => {
	return async (c, next) => {
		...
		getLogger().info`${c.req.method} ${c.req.url} ${status} ${duration}ms`;
	};
};
