...
import { findNodeById, updateNodeStatus } from "./db/nodeOperations";
...
export async function testNodeConnection(
  ...
  try {
      ...
      try {
        ...
        if (result.returnCode === 0) {
          try {
            ...
            logger.error('Failed to parse PowerShell JSON response', 'HealthCheck', {
              hostname: nodeConfig.hostname,
    ...
    logger.error('SSH connection test failed', 'HealthCheck', {
      hostname: nodeConfig.hostname,
...
export async function performInitialHealthCheck(nodeId: number): Promise<HealthCheckUpdateResult> {
  try {
    ...
    logger.error('Failed to perform initial health check', 'HealthCheck', { nodeId, error });
    return {
      success: false,
      nodeId,
      previousStatus: 'unknown',
...
export function updateNodeStatusAfterHealthCheck(
  ...
  try {
    ...
    if (updateSuccess) {
      logger.info('Node status updated after health check', 'HealthCheck', {
        nodeId,
        hostname: node.hostname,
        previousStatus,
        newStatus,
    ...
    logger.error('Failed to update node status after health check', 'HealthCheck', { nodeId, error });
    return {
      success: false,
      nodeId,
      previousStatus: 'unknown',
