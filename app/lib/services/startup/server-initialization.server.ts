...
} from "../scheduler/scheduler-system";
...
export class ServerInitializationService {
  ...
  async initialize(): Promise<void> {
    if (this.isInitialized) {
      this.logger.warn('Server already initialized', 'ServerInitialization');
      return;
    }

    this.logger.info('Starting server initialization with environment-based scheduler system', 'ServerInitialization');

    try {
      // Initialize scheduler system from environment variables
      this.schedulerSystem = await initializeAndStartSchedulersFromEnvironment();

      this.isInitialized = true;
      
      this.logger.info('Server initialization completed', 'ServerInitialization', {
        schedulerStatus: this.schedulerSystem.getStatus()
      });

    } catch (error) {
      this.logger.error('Server initialization failed', 'ServerInitialization', {
  ...
  async shutdown(): Promise<void> {
    ...
    this.logger.info('Starting graceful shutdown', 'ServerInitialization');

    try {
      // 統一スケジューラーシステムの停止
      await this.schedulerSystem.stop();

      this.logger.info('Graceful shutdown completed', 'ServerInitialization');

    } catch (error) {
      this.logger.error('Error during graceful shutdown', 'ServerInitialization', {
        error: error instanceof Error ? error.message : 'Unknown error'
      });
    } finally {
 ...
 */
export function getServerInitialization(config?: ServerInitializationConfig): ServerInitializationService {
 ...
 * サーバー初期化を実行（環境変数ベース）
 */
export async function initializeServer(config?: ServerInitializationConfig): Promise<void> {
  const service = getServerInitialization(config);
  await service.initialize();

  // プロセス終了時のクリーンアップ (Note: SchedulerSystem also handles this)
  const gracefulShutdown = async (signal: string) => {
    getLogger().info('Received shutdown signal, cleaning up...', 'ServerInitialization', {
      signal
    });
